...
[
	...
	[
		...
		{ div:["c x","tag-r_--o-call_add_user-%1-user_id-%0-"], c:
		[
			{ li:["gws b02","va"], c:[ { div:["","user_ls-users"], ev:["_dd"], c: // ls
                	[
                        	{ p:["c w17","o"], c:
                        	[ 
                        		{ u:["call_add_user","r_"] }, 
		...
		{ div:["d x"], c:[ { ac:["ay btn","call_add_ld-chan","_add_dial","x15 y gws cb tc","Dial"] }, { s:["y b savl","..."] } ] },
		{ div:["e"] }
	]},
]};
...
[
	...
	[
		...
		{ div:["d x"], ac:["","","_uvp","x y h3 cb","&Cross;"] },
		{ div:["e"] }
	]},
...
te["call_toolbar"] = { div:["ma w18",":v:activities:src_uid"], c: //  
[
	{ input:["g","call_toolbar","barr","1","radio","1"] },
	{ li:["abs mtn37 ao w13 bd"], ev:["_activity_show"], s:["w13 bd cw",""], c:[ { div:[""], c:
	[
		{ s:["c w07 x y tr",""] },
		{ s:["c w05 y ",""] },
		{ arg:["ts","",""] }, 
		// { ac:["d ay","","_call_exit","x y h3","&Cross;"] }, 
		{ div:["e"] }
	]} ]},
]};
...
te["chan_call"] = { p:["bb_","%2"], c:[ { li:[""], ev:[], c: 
[ 
...
te["chan_agent_stats"] = { c:
[
	{ s:["c w08 x y tr cb","%0"] },
	{ s:["c w08 x y tr cb","%1"] },
	{ s:["c w08 x y tr cb",":h:hms:2:0"] },
]};

te["chan_agent_cid_name"] = { s:["","%1"] };

te["chan_agent"] = { p:["","%2"], c:[ { li:[""], ev:[], c: // 
[
        { input:["g","optc","chvw0","%2","radio",""] },
        { div:["x y02 gw sh bt_ bl_ mb"], c:
        [
                { s:["c w03 x y cb",""] },      // cid_num
                { s:["c w08 x y cb",null] },    // cid_name
                { div:["c w02 x y cb","vgtk"] },      // vector
                { s:["c w12 x y s",""] },   	// cid2
		{ div:["c w27"] },
...
[
	...
	{ div:["c"], s:["xx t07 b05 cb","On Break"] }, 	 // todo: query break status
	{ div:["e"] },
]};
...
function call_popup (el, f=0)
{
	...
	nd (coll[6].childNodes[1].childNodes[1], te["activity_vw_id_"], ["","","1","","call_btns","","","1",""], r_, [9]); 	
...
function chan_status (tp, ch) 
{
	var ss = 
	{
		...
		"chan_agent":["Down","Dialing","Ringing","Waiting","Ringing","OnCall","Call Ended","Wrapup","On Hold"],
                "chan_agtk":["Down","Dialing","Ringing","IVR","In Queue","Connected","Call Ended","On Mute","On Hold"],
	...
	ch[AMI.CHAN_STATUS_TS_TXT_] = hmst (ch[st], ["","","hms","","","",""]);	// status-duration
	var ch2 = null;
	...
	if (ch2!=null && ch[AMI.CHAN_ORIG].length>0 && ch[AMI.CHAN_VECTOR]!="2") // use status of peer when autodialing
	{
		...
		ch[AMI.CHAN_STATUS_TS_TXT_] = hmst (ch2[st2], ["","","hms","","","",""]);	// status-duration
	}
}
...
function chani (tp,p,ch,ts,k_=2,top_=0)
{
	...
	{ 
		...
		"chan_agtk":[[1,AMI.CHAN_VECTOR,"::vector:0:2"]],
	...
	if (el==null)
	{
		...
		if (tp=="chan_agent") // query usr details
		{
			...
			url (el.firstChild.childNodes[1].childNodes[1], "chan_agent_cid_name", "wallonly", ("?exten="+ch[AMI.CHAN_CALLERID_NUM]+"&_c=1"));
                        ...
                        url (el.firstChild.childNodes[1].childNodes[4], "chan_agent_stats", "wallonly", ("?exten="+ch[AMI.CHAN_CALLERID_NUM]+"&stats=1"));
...
{		
	...
	for (var i=0; i<k.length; i++)  // remove closed, hangup channels
	{
		...
		console.log ("[pop] --> "+id+" "+chan_a[id].el +" | "+i+" of "+k.length)
		if (chan_a[id].el && chan_a[id].el.parentNode) 
		{
			...
			if (pe && pe.id=="vgtk") 
			{ 
				url (pe.parentNode.childNodes[4], "chan_agent_stats", "wallonly", ("?exten="+pe.parentNode.childNodes[0].innerHTML+"&stats=1"));
			}
...
function chans (o,k,ts)
{
	...
	for (var i=k.length-1; i>-1; i--)
	{
		...
		if (ch[0].length>0) 		 // agent session chan
		{
			...
			if (ch[AMI.CHAN_UNIQUEID_2].length>0 && o[ch[AMI.CHAN_UNIQUEID_2]]) 
			{
				// nb:wallboard only (affects call transfer)
				chani ("chan_agtk", el_.firstChild.childNodes[1].childNodes[2], o[ch[AMI.CHAN_UNIQUEID_2]], ts);
			}
			continue;
		}
...
// console.log ("-chan-"+ch[4]+"|"+user_cid+"|"+ch[AMI.CHAN_SIPCALLID]);		
		if (ch[4]==user_cid && ch[AMI.CHAN_SIPCALLID].length>0)
		{

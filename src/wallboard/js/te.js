...
var ND = ["u","uchk","uhilite","uval","uvw","ufn","uaudio","ucolor","uerror","usub","uchkargs","ucal","ucalv","utime", "urpt", "pivot", "uchart", "utable", "usummary", "uredirect"];
...
function uval_ (el, u, ja, cn)
{
	...
	for (var j=ja; j<vv.length; j++) 
	{
		...
		var s = "";	
		s+="<div class='y02 cd'><span>"+rz[u[0]][j]+"</span><span class='x micon'>navigate_next</span></div>";
		s+="<div>"+vv[j]+"</div>";
...
function uhilite (el, u, a, r, m)
{
	...
	var vv = txt.firstChild.firstChild.value.split (" ");
	var cn = [];
	var w = [];
	var pk = [];
	var pos = {}
	for (var i=0; i<vv.length; i++)
	{
		...
		while ((m = expr.exec (u[0])) !== null)
		{
			...
			pk[pk.length] = pi;
			// console.log ("   [uhilite] ("+vv[i]+") "+expr.lastIndex+"|"+m[0]);
		}
	}
	pk.sort ((a,b)=>{ return a-b });
...
function nd (p,o,a,r,m) 
{
	...
	for (var i=0; i<ND.length; i++)
	{
		...
		// console.log (" ND --> "+JSON.stringify (u_)+"|"+JSON.stringify (a)+"|"+JSON.stringify (m));
...
function uvpfl (pvf)
{
	...
	argv (pvf, a);
	if (p.childNodes.length>0) argv (p.firstChild, a);
	url (p, u[0], u[1], (a[".id"]+a.args));
}
...
function vpf (el)
{
	...
	ra[u[1]] = o;
	vp (p);
	console.log ("vpf: "+elvpf.id+" | "+JSON.stringify (o));
	nd (p, te[u[0]], [], [], [0]);
}
...
function _tabf (ev) 
{
	...
	var a = { ".id":"", args:"?" };
	argv (p.childNodes[1].firstChild, a); // ls | rpt args

...
function _kv ($k, &$op, &$o, &$p)
{
	...
	{ 
		...
		$op=$vv[1]; 
		$k=$vv[2];  
		if (!isset ($o[$k]) && isset ($vv[3])) $k = $vv[3]; // set default and case_activity_add
		if ($op=="")
		{
...
{
	if ($m==1) k_ft ($t, $a[0], explode (",",$v), $aa, $av);
...
function rest_uri_response ($u, $suffix, $id, &$o, &$p, &$aa, $rt)
{
	...
	if ($id!==NULL && isset ($GLOBALS[($u.$suffix."_subs")])) // recursion point
	{

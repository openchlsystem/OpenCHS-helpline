...
[
	...
	[
		...
		[
			{ div:[], c:
			[
				{ div:["d w03"], ac:["ao abs w03","client_del-clients^del","_del","cb h2",""], c:
				[
					{ s:["x y h2 tc","&Cross;"] },
...
{
	var coll = __(el,"clients_case").parentNode.childNodes;
	var contact_id = valf (r,":v:clients:contact_id");
	for (var k=0; k<2; k++)
	{
		if (coll[k].childNodes.length<2) continue;
		var coll_ = coll[k].childNodes[1].childNodes;
		for (var i=0; i<coll_.length; i++)
		{
			var el_ = _(coll_[i],"_is_client","input","name");
			if (el_ && el_.value==contact_id) el_.checked = false;
		}
	}
}

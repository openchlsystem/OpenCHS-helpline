...
te["chan_call"] = { p:["ba_ mtn1","%2"], c:[ { li:[""], ev:[], c: 
[ 
...
function call_popup (el, f=0)
{
	...
	nd (coll[6].childNodes[1].childNodes[1], te["activity_vw_id"], ["call_btns"], r_, [1]);	
	url (coll[6].childNodes[1].childNodes[1].lastChild, "activity_match", "activities", ("-1?_c=5&src="+a.src+"&src_uid="+a.src_uid+"&src_address="+r_[k["src_address"][0]]));
...
function chans (o,k,ts)
{
	...
	for (var i=k.length-1; i>-1; i--)
	{
		...
		{		
			...
			if (el) 
			{
				// console.error ("chan_agent "+ch[2]+" | "+chan_a[ch[2]]+"|"+ts)
				chan_a[ch[2]].ts=ts;

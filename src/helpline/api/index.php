<?php
include "/var/www/tz2_config.php";//
...
function _request_ ()
{
	...
	if ($u=="msg_end") // close chat/notif/etc
	{
		//if ($o["src"]=="WENI")
		{
			$api_url = $GLOBALS["API_GATEWAY_CLOSE_MSG"].$o["src_uid"]."/close/";
		        error_log ("[api]".$api_url);	
			$hdrs = array ("Content-Type: application/json");
                        $postdata = array
                        (
                                "username"=>"semaapi@sematanzania.org",
                                "password"=>"Op3nCh1ld"
                        );
		...
		return  _notify ("close?", $o["src"], $o["src_uid"], $o["src_address"], $p["auth_exten"], "close", "2");		
	}	
	...
	if ($_SERVER["REQUEST_METHOD"]=="POST")
	{
		...
		if ($u=="cases") 
		{
			...
			if ($id!=NULL)
			{
				...
				$p_ = [];
				_dup ($dup_, $o_, $p_); 
				if ($o["escalated_to_id"]==$p_["escalated_to_id"]) unset ($o["escalated_to_id"]); // unset escalated_to if no change
		...
		if (($u=="cases" || $u=="dispositions") && $rt>200 && $rt<203 && isset ($o["src"]) && $o["src"]=="call") // shrink wrapup to 60 seconds on save
                {
			$s = "wrapup?action=3&usr=".$_SESSION["cc_user_exten"];
                        $r = muu ("ami","sync?c=-1&");
                        $o_ = json_decode ($r['data'], true);
		...
		{
			$s = "read?src=".$o["src"]."&address=".$o["src_address"]."&id=".$o["src_uid"]."&";
			muu ("ati", $s); // post to queue
			
			// TODO: send reply via ati to enable reattempts
			...
			(
				...
				"chat_session"=>$o['src_uid'], 
				"chat_channel"=> $o["src"],
				"chat_source"=>"OUTBOX"//"HELPLINE"

			);
			error_log ("[postdata]".json_encode($postdata));
			kurl ($GLOBALS["API_GATEWAY_SEND_MSG"], 60, json_encode($postdata), $hdrs);
		}
